...
import { 
  ClipboardList, Settings, Trash2, Plus, X, CheckCircle2, AlertCircle,
  Users, Save, FileSpreadsheet, Upload, Download, Printer
} from 'lucide-react';
...
const Modal = ({ isOpen, title, message, onConfirm, onCancel, confirmText = "確定", cancelText = "取消", isDanger = false }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3 border-t">
          {onCancel && (
            <button onClick={onCancel} className="px-4 py-2 rounded-lg text-gray-700 bg-white border hover:bg-gray-50 transition-colors">
              {cancelText}
            </button>
          )}
          <button onClick={onConfirm} className={`px-4 py-2 rounded-lg text-white transition-colors ${isDanger ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-600 hover:bg-blue-700'}`}>
            {confirmText}
          </button>
        </div>
      </div>
...
const Toast = ({ message, type = 'success', onClose }) => {
  ...
  return (
    <div className={`fixed top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 px-6 py-3 rounded-full shadow-lg text-white z-50 animate-in slide-in-from-top-4 duration-300 ${type === 'success' ? 'bg-green-600' : 'bg-red-500'}`}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubSettings = onSnapshot(settingsRef, (snapshot) => {
      snapshot.docs.forEach(doc => {
        if (doc.id === 'subjects') setSubjects(doc.data().list || defaultSubjectsData);
        if (doc.id === 'classes') setClasses(doc.data().list || defaultClassesData);
      });
    }, (error) => console.error(error));
  ...
  const handleSubmitAbsence = () => {
    ...
    if (selectedStudents.length === 0) {
      showModal({
        title: "無缺考學生",
        message: `確定 ${classes.find(c => c.id === selectedClass)?.name} 的 ${selectedSubject} 考試「全勤」嗎？`,
        confirmText: "確定送出全勤",
        onConfirm: () => { saveAbsenceData([]); closeModal(); },
        onCancel: closeModal
      });
      return;
    }
  ...
  const handleAddSubject = async () => {
    ...
    if (subjects.includes(newSubject.trim())) return showToast('此科目已存在！', 'error');
    try {
      const newList = [...subjects, newSubject.trim()];
      await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'subjects'), { list: newList });
      setNewSubject('');
      showToast('科目新增成功');
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          snapshot.forEach(async (docSnap) => {
            await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'absences', docSnap.id));
          });
          showToast('所有紀錄已清空');
        } catch (e) {
  ...
  const handleFileUpload = (e) => {
    ...
    if (!file) return;
    if (!isSheetJsLoaded || !window.XLSX) return showToast('Excel 解析套件載入中，請稍後', 'error');
    ...
    reader.onload = async (evt) => {
      ...
      } catch (error) { showToast('檔案解析失敗', 'error'); }
    };
      ...
      <header className="bg-indigo-600 text-white shadow-md sticky top-0 z-40 print:hidden">
        <div className="max-w-5xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center">
          ...
          <div className="flex gap-1 sm:gap-2 bg-indigo-700/50 p-1 rounded-lg">
            <button onClick={() => setActiveTab('teacher')} className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'teacher' ? 'bg-white text-indigo-700 shadow' : 'text-indigo-100 hover:bg-indigo-600'}`}>
              <ClipboardList size={18} />
              <span className="hidden sm:inline">教師登記</span>
            </button>
            <button onClick={() => setActiveTab('admin')} className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'admin' ? 'bg-white text-indigo-700 shadow' : 'text-indigo-100 hover:bg-indigo-600'}`}>
              <Settings size={18} />
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="p-6 sm:p-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">考試科目</label>
                    <select value={selectedSubject} onChange={(e) => setSelectedSubject(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all appearance-none bg-slate-50 hover:bg-white">
              ...
              <div className="bg-slate-50 p-6 sm:p-8 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="text-slate-600 font-medium">已標記缺考： <span className="text-xl font-bold text-red-600">{selectedStudents.length}</span> 人</div>
                <button onClick={handleSubmitAbsence} className="w-full sm:w-auto px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2">
                  <Save size={20} />送出缺考名單
                </button>
              </div>
            </div>
              ...
              <div className="max-w-md mx-auto mt-10 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-8">
                  <div className="flex justify-center mb-6">
                    <div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600"><Settings size={32} /></div>
                  </div>
                  <h2 className="text-2xl font-bold text-center text-slate-800 mb-2">管理員驗證</h2>
                  <form onSubmit={handlePasswordSubmit}>
                    <input type="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} placeholder="請輸入密碼" className="w-full px-4 py-3 mb-4 rounded-xl border border-slate-300 focus:border-indigo-500 focus:ring-2 outline-none text-center tracking-widest text-lg" autoFocus />
                    <button type="submit" className="w-full px-4 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-md">確認登入</button>
                  </form>
                </div>
              </div>
            ) : (
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden print:shadow-none print:border-none">
                  <div className="p-6 border-b border-slate-100 flex justify-between items-center print:hidden">
                    ...
                    <div className="flex gap-2">
                      <button onClick={handleExportCSV} className="p-2 text-slate-600 hover:bg-slate-100 rounded-lg flex items-center gap-1 text-sm"><Download size={16}/> CSV</button>
                      <button onClick={() => window.print()} className="p-2 text-slate-600 hover:bg-slate-100 rounded-lg flex items-center gap-1 text-sm"><Printer size={16}/> 列印</button>
                      <button onClick={handleClearAllAbsences} className="p-2 text-red-600 hover:bg-red-50 rounded-lg flex items-center gap-1 text-sm"><Trash2 size={16}/> 清空</button>
                    </div>
                  </div>
                            ...
                            <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="p-4">
                                {record.students.length === 0 ? (
                                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">全勤</span>
                                ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print:hidden">
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div className="p-6">
                      <div className="flex gap-2 mb-6">
                        <input type="text" value={newSubject} onChange={(e) => setNewSubject(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddSubject()} placeholder="輸入新科目名稱..." className="flex-1 px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:border-indigo-500" />
                        <button onClick={handleAddSubject} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium flex items-center gap-1"><Plus size={18} /> 新增</button>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {subjects.map(subject => (
                          <div key={subject} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 rounded-lg border border-slate-200 text-slate-700">
                            <span>{subject}</span>
                            <button onClick={() => handleDeleteSubject(subject)} className="text-slate-400 hover:text-red-500 transition-colors p-0.5 rounded-md hover:bg-red-50"><X size={14} /></button>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    <div className="p-6 space-y-6">
                      <div>
                        <h3 className="text-sm font-bold text-slate-700 mb-2 flex items-center gap-2"><span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-xs">推薦</span>上傳 Excel 檔案</h3>
                        <label className="flex flex-col justify-center items-center w-full h-24 px-4 transition bg-slate-50 border-2 border-slate-300 border-dashed rounded-lg cursor-pointer hover:border-indigo-500 hover:bg-indigo-50">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="font-medium text-slate-600">點擊選擇 Excel 檔案</span>
                          </div>
                      ...
                      <div className="relative">
                        ...
                        <div className="relative flex justify-center text-sm"><span className="px-2 bg-white text-slate-400">或</span></div>
                      </div>
                      <div>
